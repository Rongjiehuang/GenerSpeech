...
output units",
    ...
    parser.add_argument(
        "--w2l-decoder", choices=["viterbi", "kenlm", "fairseqlm", "lid"], help="use a w2l decoder"
    )
...
def main(args, task=None, model_state=None):
    ...
    def build_generator(args):
        ...
        elif w2l_decoder == "lid":
            from examples.speech_recognition.lid_decoder import LidDecoder

            return LidDecoder(args, task.target_dictionary)
        else:
    ...
    with progress_bar.build_progress_bar(args, itr) as t:
        ...
        for sample in t:
            ...
            gen_timer.start()
            #print('lll:', args.dump_features)
            #print('lll:',args.dump_emissions) args.dump_emissions == None
            if args.dump_emissions:
                with torch.no_grad():
                    encoder_out = models[0](**sample["net_input"])
                    emm = models[0].get_normalized_probs(encoder_out, log_probs=True)
                    emm = emm.transpose(0, 1)
                    not_padding = 1 - encoder_out["encoder_padding_mask"].unsqueeze(dim=2).repeat(1, 1, encoder_out["encoder_out"].size(2)).float()
                    emm = torch.div(torch.sum(emm * not_padding, dim=1), torch.sum(not_padding, dim=1)).cpu().numpy().astype(np.str)

                    for i, id in enumerate(sample["id"]):
                        emissions[id.item()] = emm[i]
                        label[id.item()] = sample["target"][i].int() - 4
                    continue

            if args.dump_features:
                with torch.no_grad():
                    encoder_out = models[0](**sample["net_input"])
                    feat = encoder_out["inner_rep"].transpose(0,1)
                    not_padding = 1 - encoder_out["encoder_padding_mask"].unsqueeze(dim=2).repeat(1, 1, encoder_out["features_out"].size(2)).float()
                    feat = torch.div(torch.sum(feat * not_padding, dim=1), torch.sum(not_padding, dim=1)).cpu().numpy().astype(np.str)
                    for i, id in enumerate(sample["id"]):
                        label[id.item()] = sample["target"][i].int() - 4
                        features[id.item()] = feat[i]
                    continue
    ...
    wer = None
    if args.dump_emissions:
        fpa = open(args.dump_emissions, "w")
        emm_arr = []
        for i in range(len(emissions)):
            line = str(i) + " " + " ".join(emissions[i][4:14]) + " " + str(label[i][0].tolist())
        ...
        fo = open(args.dump_features, "w")
        feat_arr = []
        for i in range(len(features)):
            line = str(i) + " " + str(label[i][0].tolist()) + " " + " ".join(features[i])
